...
웹 UI에서:
1. "상품 수집" 섹션의 입력창에 키워드 입력
2. 정렬 옵션 선택 (정확도순, 날짜순, 가격순)
3. "수집 시작" 버튼 클릭
4. 키워드당 최대 1,000개 자동 수집
5. 중복 수집 방지: 이미 수집한 키워드는 자동 차단됨

**중복 수집 재시도**:
- 웹 UI에서는 중복 메시지에 표시된 `force=true` 힌트 참고
- API에서는 `force=true` 파라미터 사용:
  ```bash
  curl -X POST "http://localhost:8000/products/collect?query=커피&force=true"
  ```
...
2. "검색" 버튼 클릭
3. 총 검색 결과 개수 확인
4. 페이지네이션으로 모든 결과 탐색 (검색 조건 유지)
5. "전체 보기" 버튼으로 검색 초기화

### 3. 수집 이력 확인

웹 UI에서:
- "최근 수집 이력" 섹션에서 최근 10개 이력 자동 표시
- 각 항목: 키워드, 수집 개수, 정렬 방식, 수집 시간

API로:
```bash
# 최근 수집 이력
curl http://localhost:8000/products/history/recent?limit=10

# 특정 키워드 이력
curl http://localhost:8000/products/history/keyword/커피
```
